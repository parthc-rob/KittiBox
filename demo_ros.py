...
########################ROS 
class KittiBoxRosEval():
    def __init__(self):
        self.pub = rospy.Publisher('eval_image_raw',Image, queue_size = 3)
        print("Rospy publisher initialized..")
...
#            return
        ...
        rospy.Subscriber('front_usb_cam/image_raw', Image, self.kittibox_callback)
        rospy.spin()
    def kittibox_callback(self, input_image_msg):

        cv_image = bridge.imgmsg_to_cv2(input_image_msg, desired_encoding='passthrough')
        
        image = np.asarray(cv_image) #FLAGS.input_image
        ...
        self.pub.publish(bridge.cv2_to_imgmsg(output_image, encoding="passthrough")
...
if __name__ == "__main__":
    print("Initializing KittiBoxRosEval Node.......")
    rospy.init_node('kittibox_eval')
    try:
        box_pub = KittiBoxRosEval()
